...
class AutoOrderFile:
    def __init__(self):
        ...
        if self.carpeta:
            ...
            self.ordenar_carpeta(carpeta=self.carpeta)
            self.renombrar_archivos(self.carpeta)
    ...
    def ordenar_carpeta(self, lista_rutas:list=None, carpeta:str=None):
        if lista_rutas == None: lista_rutas = self.obtener_rutas_imagenes(carpeta)

        imagen_referencia = lista_rutas[0]
        rutas_iguales = [imagen_referencia]
        rutas_diferentes = []

        for ruta in lista_rutas[1:]:
            indice = self.comparar_imagenes(imagen_referencia, ruta)
            print(f"Indice entre {os.path.basename(imagen_referencia)} y {os.path.basename(ruta)}: {indice}")
            if indice > 0.85: rutas_iguales.append(ruta)
            else: rutas_diferentes.append(ruta)
        
        self.mover_archivos_a_carpeta(rutas_iguales)
        if rutas_diferentes != []: self.ordenar_carpeta(lista_rutas=rutas_diferentes)
