...
class tkFunctions:
    ...
    def grid_widgets_list(self, master:tk.Frame, widgets:list, orient:str=tk.HORIZONTAL, maximo:int=None, **kwargs):
        if not maximo or not orient: maximo = int(len(widgets)**(1/2))
        ...
        if orient == tk.VERTICAL:
            for i, widget in enumerate(widgets):
                fila, columna = (i%maximo), (i//maximo)
                widget.grid(row=fila, column=columna, sticky=tk.NSEW, **kwargs)
        else:
            for i, widget in enumerate(widgets):
                fila, columna = (i//maximo), (i%maximo)
                widget.grid(row=fila, column=columna, sticky=tk.NSEW, **kwargs)
